...
void loop() {
  ...
  uint8_t phaseValue = phase_num%19;

  long sst = micros();
  Serial.write(start_marker);
  
  Serial.write((uint8_t*)timeArray, sizeof(timeArray));
  Serial.write((uint8_t*)valArray, sizeof(valArray));
  Serial.write(phaseValue);

  
  Serial.write(end_marker);
  // long ed = micros();

  // Serial.println(String("time cost : ") + (ed-sst));
